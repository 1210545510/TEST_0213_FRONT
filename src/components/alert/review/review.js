...
export const alertReview = {
  ...
  data () {
    return {
      ...
      createrOption: {
        ...
        url: `${apiPath.alert.alertList.addAlert}` // 确认创建的请求url
      },
  ...
  // 实例创建后,进行默认数据处理
  created () {
    const getAlertId = this.$route.query.alertId
    if (getAlertId) {
      this.reviewId = getAlertId
  ...
  methods: {
    ...
    getFistReview () {
      ...
      this.$ajax({
        method: 'get',
        url: `${apiPath.alert.review.reviewFist}`,
        params: {
    ...
    doUnlink () {
      ...
      if (selectionList.length > 0) {
        ...
        return common.warningInfoTip('Please choose the alerts!')
      }
      ...
      this.$confirm('Are you sure you want to do "Unlink"?', 'Confirm', {
        ...
        this.$ajax({
          method: 'put',
          url: `${apiPath.alert.review.unbind}`,
          data: {
            ids: getAlertIds
          }
        }).then(res => {
    ...
    relateSearchEnter (kw) {
      if (!isNaN(kw)) {
        this.$router.push({name: 'alertReview', query: { alertId: kw }})
        window.location.reload()
      } else {
